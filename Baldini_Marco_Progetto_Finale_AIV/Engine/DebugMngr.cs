...
namespace Baldini_Marco_Progetto_Finale_AIV
{
    static class DebugMngr
    {
        ...
        public static bool ShouldDraw { get; private set;}
        static DebugMngr()
        {
        ...
        static public void SetShouldDraw(bool value)
        {
            ShouldDraw = value;
        }
        ...
        public static void Draw()
        {
            ...
            {
                if (items[i].RigidBody.Collider != null && items[i].RigidBody.IsActive)
                {
                    OpenTK.Vector4 col;

                    if (items[i] is CircleCollider)
                    {
                        col = new OpenTK.Vector4(1.0f, 1.0f, 0.0f, 0.2f);
                    }
                    else
                    {
                        col = new OpenTK.Vector4(0.0f, 1.0f, 1.0f, 0.2f);
                    }
